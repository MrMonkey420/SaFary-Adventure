...
bool Window::Awake(pugi::xml_node& config)
{
	...
	else
	{
		...
		bool fullscreen = config.child("fullscreen").attribute("value").as_bool(); //todo FALSE dentro de los paréntesis
		...
		width = config.child("resolution").attribute("width").as_int(1280); //SUSSY1 (treure números???)
		height = config.child("resolution").attribute("height").as_int(720); // dels 3
		scale = config.child("resolution").attribute("scale").as_int(1); // aquest tb
